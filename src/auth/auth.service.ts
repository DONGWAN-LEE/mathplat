...
import { sha256Hash, constantTimeCompare, generateSecureRandomString, hashPassword, verifyPassword } from '../common/utils';
...
export class AuthService {
   ...
   *
   * @description
   * 이메일+비밀번호로 유저를 생성하고 세션 및 JWT 토큰을 발급합니다.
   * googleId는 null로 설정되며, 비밀번호는 scrypt 해시로 저장됩니다.
   * DB 유니크 제약조건으로 soft-deleted 이메일 충돌도 감지합니다.
   * AUTH_DEV_MODE=true일 때만 호출되어야 합니다.
   *
   * 쿼리 수: 2 (user create + session create)
   *
   * @param email - 이메일 주소
   * @param password - 비밀번호 (평문)
   * @param name - 사용자 이름
   * @returns 유저 정보, 토큰 페어, 신규 가입 여부를 포함하는 응답
   * @throws ConflictException 이미 등록된 이메일인 경우 (soft-deleted 포함)
   */
  async handleDevSignup(
    email: string,
    password: string,
    name: string,
    ...
    try {
      ...
      if (
        ...
        throw new ConflictException({
          success: false,
          error: { code: 'REQ_002', message: 'Email is already registered' },
        });
      }
      throw error;
    }

    this.logger.info('개발모드 신규 유저 생성', { userId: user.id, email });
   ...
   *
   * @description
   * 이메일로 유저를 조회하고 scrypt 해시로 비밀번호를 검증합니다.
   * 검증 성공 시 기존 세션을 배치 무효화한 후 새 세션을 생성하고 JWT 토큰을 발급합니다.
   * AUTH_DEV_MODE=true일 때만 호출되어야 합니다.
   *
   * 쿼리 수: 3 (user find + session invalidate + session create)
   *
   * @param email - 이메일 주소
   * @param password - 비밀번호 (평문)
   * @returns 유저 정보, 토큰 페어를 포함하는 응답
   * @throws UnauthorizedException 이메일/비밀번호가 일치하지 않는 경우
   */
  async handleDevLogin(
    ...
    this.logger.info('개발모드 로그인 성공', { userId: user.id, email });
    ...
    return this.createAuthenticatedSession(user, false);
  }

  /**
   * 인증된 세션을 생성하고 JWT 토큰 페어를 반환합니다
   *
   * @description
   * 세션 ID를 사전 생성하여 단일 쿼리로 세션과 refreshToken 해시를 함께 저장합니다.
   * Redis 캐시에 유저 정보도 저장합니다.
   *
